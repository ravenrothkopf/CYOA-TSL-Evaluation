...
// at top, just one recordKey:
function recordKey() {
  apiKey = document.getElementById("input_APIKey").value.trim();
  // pull the model *right here* too:
  selectedModel = document.getElementById("myDropdown").value;
  
  console.log("🔑 API key set, length =", apiKey.length);
  console.log("🤖 Model selected =", selectedModel);

  testAPIKey();   // now test with a real model
}

// only one listener, on your “submit API key” button:
document.getElementById('submitAPIKey')
        .addEventListener("click", recordKey);

// remove all other submitButton listeners…
...
async function openAIFetchAPI(promptMessages, numChoices = 1, stopSeqs = []) {
  ...
  if (!selectedModel) throw new Error("No model selected – choose GPT-4 or 3.5 first.");
...
async function testAPIKey() {
  try {
    ...
    document.getElementById('log').innerHTML +=
      `<li><strong>API Test:</strong> ${fact}</li>`;
  } catch (err) {
    console.error("❌ API Test failed:", err);
    document.getElementById('log').innerHTML +=
      `<li style="color:red;"><strong>API Test failed:</strong> ${err.message}</li>`;
  }
}
